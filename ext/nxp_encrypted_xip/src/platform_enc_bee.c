 ...
 */
static prdb_t prdb_template __attribute__((aligned)) = { .tagl = PRDB_TAGL,
		.tagh = PRDB_TAGH, .version = PRDB_VERSION, .fac_region_count = 1,
		.encrypt_region_info = { .region_1_start = FLASH_ADDR, /* Only region 1 - rest is region 0 */
		.region_1_end = FLASH_ADDR + BEE_REGION_SIZE, .aes_mode =
				kBEE_AesCtrMode, .lock_option = 0, .aes_ctr_nonce = {
				0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF }, },
		.fac_region_2 = { .start = FLASH_ADDR, .end = FLASH_ADDR
				+ BEE_REGION_SIZE, .mode = 0, }, };
...
static status_t fac_regions_setup(prdb_t *prdb) {
	...
	if ((prdb->fac_region_0.start > BOOT_FLASH_BASE)
		...
		IOMUXC_GPR->GPR11 |= IOMUXC_GPR_GPR11_M7_APC_AC_R0_CTRL(
				prdb->fac_region_0.mode);
	...
	if ((prdb->fac_region_1.start > BOOT_FLASH_BASE)
		...
		IOMUXC_GPR->GPR11 |= IOMUXC_GPR_GPR11_M7_APC_AC_R1_CTRL(
				prdb->fac_region_1.mode);
	...
	if ((prdb->fac_region_2.start > BOOT_FLASH_BASE)
		...
		IOMUXC_GPR->GPR11 |= IOMUXC_GPR_GPR11_M7_APC_AC_R2_CTRL(
				prdb->fac_region_2.mode);
	...
	if ((prdb->fac_region_3.start > BOOT_FLASH_BASE)
		...
		IOMUXC_GPR->GPR11 |= IOMUXC_GPR_GPR11_M7_APC_AC_R3_CTRL(
				prdb->fac_region_3.mode);
