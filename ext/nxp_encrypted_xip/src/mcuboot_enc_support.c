...
static status_t load_image(enc_data_t *data, uint8_t *nonce) {
	...
	while (bytes_copied < hdr_size) {
		...
		if (platform_enc_encrypt_data(flash_addr + cur_off, nonce,
				(uint8_t*) buffer, (uint8_t*) buffer, CHUNK_SIZE) != 0) {
	...
	while (bytes_copied < tlv_off) {
		...
		if (bytes_copied + chunk_sz > tlv_off) {
			...
			if (blk_sz > 0) {
				rc = decrypt(&data->ctx, bytes_copied - hdr_size, blk_sz,
						blk_off, (uint8_t*) buffer);
			}
		} else {
		...
		if (platform_enc_encrypt_data(flash_addr + cur_off, nonce,
				(uint8_t*) buffer, (uint8_t*) buffer, chunk_sz) != 0) {
			BOOT_LOG_INF("Encryption failed");
			goto error;
		}
...
status_t mcuboot_process_encryption(struct boot_rsp *rsp) {
	...
	if (find_and_load_aes_key(&enc_data) != kStatus_Success) {
		BOOT_LOG_INF("Invalid image: loading AES-CTR key failed");
		goto error;
	}
	...
	if (status == kStatus_Success) {
		...
		BOOT_LOG_INF(
				"Comparing the image against reference chosen by MCUBoot...");
		if (active_slot == enc_data.slot) {
			if (platform_enc_cfg_init(boot_flash_meta_map, (uint8_t*) nonce)
					== kStatus_Success) {
	...
	if (platform_enc_cfg_write(boot_flash_meta_map, enc_data.slot)
			!= kStatus_Success)
		goto error;
	...
	finish: bootutil_aes_ctr_drop(&enc_data.ctx);
	...
	error: bootutil_aes_ctr_drop(&enc_data.ctx);
